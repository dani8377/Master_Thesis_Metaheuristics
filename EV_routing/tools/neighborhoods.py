...
def two_opt(route: list[str], data: ProblemData, ev_params: EVParameters) -> list[str]:
    """Reverse a random interior subsequence (works on any node type)."""
    new_route = route[:]
    if len(new_route) < 4:
        return new_route
...
def insert_charging_station(route: list[str], data: ProblemData, ev_params: EVParameters) -> list[str]:
    ...
    if not station_ids:
        return new_route
    station = random.choice(station_ids)
    pos = random.randint(1, len(new_route) - 1)
    new_route.insert(pos, station)
    return new_route
...
def replace_charging_station(route: list[str], data: ProblemData, ev_params: EVParameters) -> list[str]:
    """Replace a charging station already in the route with a different one."""
    new_route = route[:]
...
def move_charging_station(route: list[str], data: ProblemData, ev_params: EVParameters) -> list[str]:
    """Remove a charging station from its current position and reinsert elsewhere."""
    new_route = route[:]
    station_ids = set(data.stations["Node ID"].tolist())
    positions = [i for i, n in enumerate(new_route) if n in station_ids]
    if not positions:
...
def repair_battery_violation(route: list[str], data: ProblemData, ev_params: EVParameters) -> list[str]:
    ...
    If no reachable station exists, return the route unchanged.
    """
    new_route = route[:]
    ...
    for i in range(len(new_route) - 1):
        ...
        if battery - energy < 0:
            # Find nearest station reachable from origin with current battery
...
def generate_neighbor(route: list[str], data: ProblemData, ev_params: EVParameters) -> list[str]:
    """
    Randomly select a neighborhood move. EV-specific moves that require
    existing stations in the route are only included when the route has them.
    """
    station_ids = set(data.stations["Node ID"].tolist())
